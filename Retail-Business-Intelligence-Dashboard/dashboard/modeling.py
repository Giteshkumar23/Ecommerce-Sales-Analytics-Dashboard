...
# Constants
CONFIG_FILE = "scripts/sql/config/db_config.json"
PROCESSED_DATA_PATH = "scripts/data/processed"
SCHEMA_SCRIPT_PATH = "scripts/sql/dw_schema.sql"
TABLES = {
...
def load_csv_to_postgres(table_name, csv_filename):
    ...
    try:
        df = pd.read_csv(
            csv_path,
            encoding="utf-8",
            parse_dates=["cart_date"] if table_name == "cart_dimension" else []
        )
